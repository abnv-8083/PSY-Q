...
serve(async (req) => {
    ...
    try {
        ...
        if (action === 'student-verify') {
            const { email, otp, signupData } = payload

            if (!signupData) {
                return new Response(JSON.stringify({ error: 'Missing signup data' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 })
            }

            const cleanOtp = otp.trim()
            const isExpired = new Date() > new Date(signupData.otp_expires_at)

            if (signupData.otp_code !== cleanOtp) {
            ...
            // Create user in database ONLY NOW
            ...
            return new Response(JSON.stringify({
                user: { id: newStudent.id, email: newStudent.email, full_name: newStudent.full_name, role: 'student' }
            }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 200
            })
