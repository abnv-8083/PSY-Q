...
const Psychospiritual = () => {
  ...
  const handleBookNow = (pkg) => {
    setSelectedPackage({ ...pkg, serviceName: 'Psychospiritual and meditation' });
    setModalOpen(true);
  };
  const packages = [
    {
            ...
            <Grid item xs={12} sm={6} md={4} key={index} sx={{ display: 'flex' }}>
              <Paper sx={{
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2, gap: 1, flexWrap: 'wrap' }}>
                  <Typography variant="h5" sx={{ fontWeight: 700, color: '#11151cff', fontSize: { xs: '1.1rem', sm: '1.25rem', md: '1.5rem' } }}>
                    {pkg.name}
                  </Typography>
                  <Chip 
                    label={pkg.discount}
                ...
                <Box sx={{ flexGrow: 1, mb: 3 }}>
                  {pkg.features.map((feature, featureIndex) => (
                    <Box key={featureIndex} sx={{ display: 'flex', mb: { xs: 1.5, sm: 2 }, alignItems: 'flex-start' }}>
                      ...
                      <Typography sx={{ color: '#11151cff', fontSize: { xs: '0.8rem', sm: '0.9rem' }, lineHeight: 1.5 }}>
                        {feature}
                      </Typography>
                    </Box>
                  ))}
...
export default Psychospiritual;
